...
info( '*** Adding hardware interface', 'tapa', 'to host',
      r1.name, '\n' )
_intf = Intf( 'tapa', node=r1, ip='192.168.3.1/24' )
...
r1.cmd("ip route add default via 192.168.3.1 dev tapa")
...
# start inet
subprocess.run(['inet -f omnetpp.ini -c Mininet &'], user=user, group=group, env=saved_env, shell=True)
